// script.js - 666LF Win v4 (PT-BR)
const APP = {
  init(){
    this.users = JSON.parse(localStorage.getItem('lf_users')||'{}');
    this.current = null;
    this.bindUI();
    // show preloader then app
    setTimeout(()=>{ document.getElementById('preloader').classList.add('hidden'); document.getElementById('app').classList.remove('hidden'); this.showAuth(); }, 1400);
  },
  bindUI(){
    ...
    document.getElementById('topUp').onclick = ()=>{ if(this.current){ this.current.balance+=100; this.save(); this.updateUI(); this.play('coin'); alert('+100 coins (demo)'); } };
  ...
  play(name){
    try{ const s = document.getElementById('s_'+name); if(s){ s.currentTime=0; s.play(); } }catch(e){}
  }
};
